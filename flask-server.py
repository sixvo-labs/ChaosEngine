...
def temperature():
    ...
    temperature_now = 'Temperature ' + t + degree_symbol + ' recorded at ' + ti$
...
def humidity():
    ...
    h = str(round(h, 6))
    humidity_now = 'Humidity: ' + h + '% recorded at' + time.strftime("%H:%M:%S$
    return humidity_now
...
def pressure():
    ...
    p = str(round(p, 6))
    pressure_now = 'Pressure ' + p + 'inHg recorded at ' + time.strftime("%H:%M$
    return pressure_now
...
def snapshot():
    ...
    snapshot = "{" + "snapshot: " + time.strftime("%H:%M:%S", time.localtime())$
               ", data: " str(snap) + "}"
    return snapshot

    #print("{0},{1},{2},{3},{4},{5},{6},{7},{8} ".format(t, h, p, pitch, roll, $
